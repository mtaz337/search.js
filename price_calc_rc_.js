...
class PriceCalculator extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    document.getElementById("order_total").value = Math.ceil(
      Number(total_price)
    );
  }
  ...
  async getJSONOrder() {
    ...
    let backPackServiceFee =
      (checkoutPrice * this.CALC_RATES.BACKPACK_SERVICE_FEE_PERCENT) / 100;
    let backpackCheckoutPrice =
      checkoutPrice +
      (checkoutPrice * this.CALC_RATES.BACKPACK_US_SALES_TAX_PERCENT) / 100;
    let airbringrProductPrice =
      backpackCheckoutPrice * this.EXCHANGE_RATES.USD_TO_BDT;
    ...
    let dimentionalWeightInKg =
      ((prevLength * prevWidth * prevHeight) / 166) *
      16 *
      this.EXCHANGE_RATES.OZ_TO_KG;
    let dimentionalWeightInLb =
      dimentionalWeightInKg * this.EXCHANGE_RATES.KG_TO_POUND;
    ...
    let airbringrTravellerFee =
      maxWeight *
      this.CALC_RATES.AIRBRINGR_TRAVELLER_FEE_USD_PER_KG *
      this.EXCHANGE_RATES.USD_TO_BDT;
    maxWeightPound = maxWeight * this.EXCHANGE_RATES.KG_TO_POUND;
    backpackWeightFee =
      maxWeightPound * this.CALC_RATES.BACKPACK_TRAVELLER_FEE_PER_POUND;
    ...
    let amountToReduce =
      airbringrProductPrice +
      airbringrTravellerFee +
      (this.CALC_RATES.SAVINGS_FROM_BACKPACK_PERCENT / 100) * backpackTotal;
    let airbringrServiceFee = backpackTotal - amountToReduce;
    airbringrServiceFee =
      airbringrServiceFee < 0 ? airbringrServiceFee * -1 : airbringrServiceFee;
    let CRR_IN_BDT = airbringrServiceFee + airbringrTravellerFee;
    let CRR_IN_USD = CRR_IN_BDT / this.EXCHANGE_RATES.USD_TO_BDT;
    let RRR_IN_BDT =
      maxWeight *
      this.CALC_RATES.AIRBRINGR_RRR_USD_PER_KG *
      this.EXCHANGE_RATES.USD_TO_BDT;
    let crrReached = CRR_IN_USD / maxWeight;
    let airbringrServiceFeePercent =
      (airbringrServiceFee / airbringrProductPrice) * 100;
    let airbringrTravelerFeePercent =
      (airbringrTravellerFee / airbringrProductPrice) * 100;
    let airbringrTotal =
      airbringrProductPrice + airbringrTravellerFee + airbringrServiceFee;
    ...
    let productPriceBeforeDiscount =
      initialJsonResponse.jsonOrder.price_before_discount;
  ...
  handleBasePriceChange(price) {
    let existingShippingCharge = Number(
      parseFloat(this.state.productShippingCharge).toFixed(2)
    );
    let newCheckoutPrice =
      existingShippingCharge + Number(parseFloat(price).toFixed(2));
    let backpackServiceFee =
      (newCheckoutPrice * this.CALC_RATES.BACKPACK_SERVICE_FEE_PERCENT) / 100;
    let backpackCheckoutPrice =
      newCheckoutPrice +
      (newCheckoutPrice * this.CALC_RATES.BACKPACK_US_SALES_TAX_PERCENT) / 100;

    let airbringrProductPrice = Math.ceil(
      backpackCheckoutPrice * this.EXCHANGE_RATES.USD_TO_BDT
    );
    let sellerTakes = backpackCheckoutPrice * this.EXCHANGE_RATES.USD_TO_BDT;

    let weightFee = Number(
      parseFloat(this.state.backpricepackWeightFeeUSD).toFixed(2)
    );
    var backpackSD = backpackServiceFee - weightFee;
    backpackSD = backpackSD < 0 ? backpackSD * -1 : backpackSD;
    let backpackTakes = backpackSD * this.EXCHANGE_RATES.USD_TO_BDT;

    let travellerTakes = Number(
      parseFloat(this.state.travellerTakesBDT).toFixed(2)
    );
    let backpackTotal = sellerTakes + travellerTakes + backpackTakes;
    ...
    let airbringrServiceFee =
      (newAirbringrServiceFeePercent / 100) *
      Number(parseFloat(airbringrProductPrice).toFixed(2));
    ...
    let CRR_IN_BDT =
      airbringrServiceFee +
      Number(parseFloat(this.state.airbringrTravellerFeeBDT).toFixed(2));
    let CRR_IN_USD = CRR_IN_BDT / this.EXCHANGE_RATES.USD_TO_BDT;
    let RRR_IN_BDT =
      Number(parseFloat(this.state.backpackWeightInKg).toFixed(2)) *
      this.CALC_RATES.AIRBRINGR_RRR_USD_PER_KG *
      this.EXCHANGE_RATES.USD_TO_BDT;

    let crrReached =
      CRR_IN_USD / Number(parseFloat(this.state.backpackWeightInKg).toFixed(2));
    ...
    let airbringrTotal =
      airbringrProductPrice +
      Number(parseFloat(this.state.travellerTakesBDT).toFixed(2)) +
      airbringrServiceFee;
  ...
  handleBaseWeightChange(weight) {
    ...
    let backPackServiceFee = Number(
      parseFloat(this.state.backpackServiceFeeUSD).toFixed(2)
    );
    var backpackSD = backPackServiceFee - backpackWeightFee;
    backpackSD = backpackSD < 0 ? backpackSD * -1 : backpackSD;
    let backpackTakes = backpackSD * this.EXCHANGE_RATES.USD_TO_BDT;
    ...
    let newAirbringrServiceFeePercent;
    let existingBasePrice = Number(
      parseFloat(this.state.productBasePrice).toFixed(2)
    );
    if (isReseller || isHighgValueShopper || isSilentShopper) {
      newAirbringrServiceFeePercent = 14.5;
    } else {
    ...
    let airbringrServiceFee =
      (newAirbringrServiceFeePercent / 100) *
      Number(parseFloat(this.state.airbringrProductPriceBDT).toFixed(2));
  ...
  handleShippingWeightChange(weightCharge) {
    ...
    let newCheckoutPrice =
      existingBasePrice + Number(parseFloat(weightCharge).toFixed(2));
    let backpackServiceFee =
      (newCheckoutPrice * this.CALC_RATES.BACKPACK_SERVICE_FEE_PERCENT) / 100;
    let backpackCheckoutPrice =
      newCheckoutPrice +
    ...
    let backpackWeightFee = Number(
      parseFloat(this.state.backpackWeightFeeUSD).toFixed(2)
    );
    var backpackSD = backpackServiceFee - backpackWeightFee;
    backpackSD = backpackSD < 0 ? backpackSD * -1 : backpackSD;
    let backpackTakes = backpackSD * this.EXCHANGE_RATES.USD_TO_BDT;
  ...
  handleRequiredAfChange(percent) {
    ...
    let airbringrTotal =
      ...
      airbringrServiceFee;
    let backpackSavings =
      Number(parseFloat(this.state.backpackTotalBDT).toFixed(2)) -
      airbringrTotal;
    let savingsPercent =
      (backpackSavings /
        Number(parseFloat(this.state.backpackTotalBDT).toFixed(2))) *
  ...
  handleLengthInchChange(length) {
    ...
    let newDimentionalWeightInKg =
      ((length * prevWidth * prevHeight) / 166) *
      16 *
      this.EXCHANGE_RATES.OZ_TO_KG;
    let newDimentionalWeightInLb =
      newDimentionalWeightInKg * this.EXCHANGE_RATES.KG_TO_POUND;
  ...
  handleWidthInchChange(width) {
    ...
    let newDimentionalWeightInKg =
      ((prevLength * width * prevHeight) / 166) *
      16 *
      this.EXCHANGE_RATES.OZ_TO_KG;
    let newDimentionalWeightInLb =
      newDimentionalWeightInKg * this.EXCHANGE_RATES.KG_TO_POUND;
  ...
  handleHeightInchChange(height) {
    ...
    let newDimentionalWeightInKg =
      ((prevLength * prevWidth * height) / 166) *
      16 *
      this.EXCHANGE_RATES.OZ_TO_KG;
    let newDimentionalWeightInLb =
      newDimentionalWeightInKg * this.EXCHANGE_RATES.KG_TO_POUND;
      ...
      return (
        <div>
              ...
              <div className="m-section__content">
                <form className="">
                  <div className="form-group m-form__group">
                    <h5 style={{ width: 125, justifyContent: "left" }}>
                      Product Name
                    </h5>
                    <input
                      type="text"
                      ...
                      id="product_title"
                      name="product_url"
                      defaultValue={this.state.jsonOrder.product_title}
                      style={{ fontSize: 14 }}
                    />
                  </div>
                  <div className="form-group m-form__group">
                    ...
                    <input
                      ...
                      defaultValue={this.state.jsonOrder.product_main_img}
                      style={{ fontSize: 14 }}
                    />
                  </div>
                  <div className="form-group m-form__group">
                    <h5 style={{ width: 250, justifyContent: "left" }}>
                      Product Origin
                    </h5>
                    <input
                      type="hidden"
                      className="form-control col-md-12"
                      name="product_origin"
                      defaultValue={this.state.jsonOrder.origin}
                      style={{ fontSize: 14 }}
                      readonly="true"
                      disabled="true"
                    />
                    ...
                    <div
                      className="form-control btn btn-light col-md-2 change-origin"
                      style={{ float: "left" }}
                    >
                      Change
                    </div>
                  ...
                  <div className="form-group m-form__group">
                    ...
                    <input
                      type="text"
                      className="form-control col-md-10 shoppernotefield"
                      name="customer_note"
                      defaultValue={this.state.jsonOrder.customer_note}
                      style={{ fontSize: 14, float: "left" }}
                      disabled="disabled"
                    />
                    <a
                      className="form-control btn btn-light col-md-2 editshppernote"
                      style={{ float: "right" }}
                    >
                      Edit
                    </a>
                ...
                <h5 style={{ marginTop: 60, marginBottom: 5 }}>
                  {" "}
                  Weight : (KG){" "}
                  {Number(parseFloat(this.state.backpackWeightInKg).toFixed(2))}
                </h5>
              ...
              <div className="m-section__content">
                <form className="form-inline">
                  <div className="container">
                    <div className="row">
                      <div
                        className="col-md-2"
                        style={{ paddingTop: 5, paddingLeft: 0 }}
                      >
                        <h5 style={{ justifyContent: "left" }}>
                          Base
                          <br /> Price:{" "}
                        </h5>
                      </div>
                      <div className="col-md-3" style={{ padding: 0 }}>
                        ...
                        >
                          <input
                            ...
                            name="base_price_usd"
                            defaultValue={
                              this.state.jsonOrder.product_base_price_usd
                            }
                            step="0.01"
                            onChange={(e) =>
                              this.handleBasePriceChange(e.target.value || 0)
                            }
                            style={{ fontSize: 14 }}
                          />
                        </div>
                      ...
                      <div className="col-md-3" style={{ padding: 0 }}>
                        ...
                        >
                          <input
                            ...
                            defaultValue={
                              this.state.jsonOrder.price_before_discount
                            }
                            step="0.01"
                            style={{ fontSize: 14 }}
                          />
                  ...
                  >
                    ...
                    <input
                      ...
                      name="base_weight_kg"
                      defaultValue={this.state.jsonOrder.product_weight_kg}
                      step="0.01"
                      onChange={(e) =>
                        this.handleBaseWeightChange(e.target.value || 0)
                      }
                      style={{ fontSize: 14 }}
                    />
                  </div>
                  <div
                    ...
                    style={{ height: 50 }}
                  >
                    <h5 style={{ width: 180, justifyContent: "left" }}>
                      Shipping Weight (OZ)
                    </h5>
                    <input
                      type="text"
                      ...
                      name="base_weight_oz"
                      defaultValue={
                        this.state.jsonOrder.product_weight_kg *
                        this.EXCHANGE_RATES.KG_TO_OZ
                      }
                      step="0.01"
                      onChange={(e) =>
                        this.handleOzToKgChange(e.target.value || 0)
                      }
                      style={{ fontSize: 14 }}
                    />
                  </div>
                  ...
                  <div className="container">
                    <div className="row">
                      <div
                        className="col-md-1"
                        style={{ paddingTop: 15, paddingLeft: 0 }}
                      >
                      ...
                      <div className="col-md-2" style={{ padding: 0 }}>
                        ...
                        >
                          <input
                            type="text"
                            className="form-control col-md-12"
                            name="base_length_inch"
                            defaultValue={
                              this.state.jsonOrder.product_length_in
                            }
                            step="0.01"
                            onChange={(e) =>
                              this.handleLengthInchChange(e.target.value || 0)
                            }
                            style={{ fontSize: 14 }}
                          />
                        </div>
                      ...
                      <div className="col-md-2" style={{ padding: 0 }}>
                        ...
                        >
                          <input
                            type="text"
                            className="form-control col-md-12"
                            name="base_width_inch"
                            defaultValue={this.state.jsonOrder.product_width_in}
                            step="0.01"
                            onChange={(e) =>
                              this.handleWidthInchChange(e.target.value || 0)
                            }
                            style={{ fontSize: 14 }}
                          />
                        </div>
                      ...
                      <div className="col-md-2" style={{ padding: 0 }}>
                        ...
                        >
                          <input
                            type="text"
                            className="form-control col-md-12"
                            name="base_height_inch"
                            defaultValue={
                              this.state.jsonOrder.product_height_in
                            }
                            step="0.01"
                            onChange={(e) =>
                              this.handleHeightInchChange(e.target.value || 0)
                            }
                            style={{ fontSize: 14 }}
                          />
                        </div>
                  ...
                  >
                    ...
                    <input
                      type="text"
                      className="form-control col-md-5"
                      name="shipping_charge_usd"
                      defaultValue="0"
                      step="0.01"
                      onChange={(e) =>
                        this.handleShippingWeightChange(e.target.value || 0)
                      }
                      style={{ fontSize: 14 }}
                    />
                  </div>
                  ...
                  >
                    ...
                    <input
                      ...
                      type="text"
                      className="form-control col-md-5"
                      name="backpack_savings_percent"
                      defaultValue={Number(
                        parseFloat(this.state.backpackSavingsPercent).toFixed(2)
                      )}
                      step="0.01"
                      onChange={(e) =>
                        this.handleBackpackSavingsPercentChange(
                          e.target.value || 0
                        )
                      }
                      style={{ fontSize: 14 }}
                  ...
                  >
                    ...
                    <input
                      ...
                      defaultValue={Number(
                        parseFloat(
                          this.state.airbringrServiceFeePercent
                        ).toFixed(2)
                      )}
                      step="0.01"
                      onChange={(e) =>
                        this.handleRequiredAfChange(
                          e.target.value ||
                            this.state.airbringrServiceFeePercent
                        )
                      }
                  ...
                  <div style={{ display: "none" }}>
                    <h5 style={{ padding: 5, marginBottom: 5 }}>
                      {" "}
                      Dimentional Weight: (LB){" "}
                      {this.state.productDimentionalWeightInLb.toFixed(4)}{" "}
                    </h5>
                    <h5 style={{ padding: 5, marginBottom: 5 }}>
                      {" "}
                      Dimentional Weight: (KG){" "}
                      {this.state.productDimentionalWeightInKg.toFixed(4)}{" "}
                    </h5>
                  </div>
                </form>

                <h5
                  style={{ paddingBottom: 10, paddingTop: 30, marginBottom: 5 }}
                >
                  {" "}
                  Savings from BP :{" "}
                  {Number(parseFloat(this.state.backpackSavingsBDT).toFixed(2))}
                </h5>
                <h5 style={{ paddingBottom: 10, marginBottom: 5 }}>
                  {" "}
                  BP Total (৳) :{" "}
                  {Number(parseFloat(this.state.backpackTotalBDT).toFixed(2))}
                </h5>
                <div style={{ display: "none" }}>
                  ...
                  <h5 style={{ padding: 5, marginBottom: 5 }}>
                    ...
                    {this.EXCHANGE_RATES.USD_TO_BDT.toFixed(2)}
                  </h5>
                  <h5 style={{ padding: 5, marginBottom: 5 }}>
                    {" "}
                    AirBringr Traveller Fee $/KG : $
                    {this.CALC_RATES.AIRBRINGR_TRAVELLER_FEE_USD_PER_KG.toFixed(
                      2
                    )}
                  </h5>
                  <h5 style={{ padding: 5, marginBottom: 5 }}>
                    {" "}
                    AirBringr RRR USD/KG : $
                    {this.CALC_RATES.AIRBRINGR_RRR_USD_PER_KG.toFixed(2)}
                  </h5>

                  <h5 style={{ padding: 5, marginBottom: 5 }}>
                    {" "}
                    Backpack US Sales Tax :{" "}
                    {this.CALC_RATES.BACKPACK_US_SALES_TAX_PERCENT.toFixed(2)} %
                  </h5>
                  <h5 style={{ padding: 5, marginBottom: 5 }}>
                    {" "}
                    Backpack Service Fee :{" "}
                    {this.CALC_RATES.BACKPACK_SERVICE_FEE_PERCENT.toFixed(2)} %
                  </h5>
                  <h5 style={{ padding: 5, marginBottom: 5 }}>
                    {" "}
                    Backpack Traveller Fee $/LB : $
                    {this.CALC_RATES.BACKPACK_TRAVELLER_FEE_PER_POUND.toFixed(
                      2
                    )}{" "}
                  </h5>
                </div>
              ...
              <div className="m-section__content">
                    ...
                    <tr>
                      ...
                      >
                        <h5>
                          ...
                          Base Price :{" "}
                          <span
                            id="rct_airbringr_product_price"
                            style={{ float: "right" }}
                          >
                            {Number(
                              parseFloat(
                                this.state.airbringrProductPriceBDT
                              ).toFixed(2)
                            )}
                          </span>
                        </h5>
                      </th>
                      <td rowSpan="3" style={{ borderTop: 0 }}>
                        <h5 style={{ paddingTop: 50 }}>
                          ...
                          >
                            {Number(
                              parseFloat(this.state.airbringrTotalBDT).toFixed(
                                2
                              )
                            )}
                          </span>
                    ...
                    <tr>
                      <th style={{ borderTop: 0 }}>
                        <h5>
                          ...
                          TF :
                          <span
                            id="rct_airbringr_traveller_fee"
                            style={{ float: "right" }}
                          >
                            {Number(
                              parseFloat(
                                this.state.airbringrTravellerFeeBDT
                              ).toFixed(2)
                            )}
                          </span>
                        </h5>{" "}
                    ...
                    <tr>
                      <th style={{ borderTop: 0 }}>
                        <h5>
                          ...
                          AF :{" "}
                          <span
                            id="rct_airbringr_service_fee"
                            style={{ float: "right" }}
                          >
                            {Number(
                              parseFloat(
                                this.state.airbringrServiceFeeBDT
                              ).toFixed(2)
                            )}
                          </span>
                        </h5>
                ...
                <div>
                  ...
                  >
                    ...
                    AF (%) :{" "}
                    {Number(
                      parseFloat(this.state.airbringrServiceFeePercent).toFixed(
                        2
                      )
                    )}
                  </h5>
                  ...
                  <h5 style={{ padding: 10, marginBottom: 5, display: "none" }}>
                    {" "}
                    RRR : ($){" "}
                    {this.CALC_RATES.AIRBRINGR_RRR_USD_PER_KG.toFixed(2)}
                  </h5>
                  <h5 style={{ padding: 10, marginBottom: 5, display: "none" }}>
                    {" "}
                    RRR : (৳) {Number(parseFloat(this.state.rrrBDT).toFixed(2))}
                  </h5>
                  <h5
                    style={{
                      padding: 10,
                  ...
                  >
                    {" "}
                    CRR (%) :{" "}
                    <span id="rct_crr_reached">
                      {Number(parseFloat(this.state.crrReached).toFixed(2))}
                    </span>
                  </h5>
                  <h5 style={{ padding: 10, marginBottom: 5, display: "none" }}>
                    {" "}
                    CRR : (৳){" "}
                    <span id="rct_crr_bdt">
                      {Number(parseFloat(this.state.crrBDT).toFixed(2))}{" "}
                    </span>
                  </h5>
                  <h5 style={{ padding: 10, marginBottom: 5 }}>
                    {" "}
                    CRR ($) :{" "}
                    <span id="rct_crr_usd">
                      {Number(parseFloat(this.state.crrUSD).toFixed(2))}
                    </span>
                  </h5>
                  <h5 style={{ padding: 10, marginBottom: 5 }}>
                    {" "}
                    ...
                    {Number(
                      parseFloat(
                        this.state.airbringrTravelerFeePercent
                      ).toFixed(2)
                    )}
                  </h5>
                </div>
          ...
          <div className="row">
            ...
            <div className="col-md-4">
              <div className="m-section__content"></div>
            </div>
            ...
            <div className="col-md-4" style={{ display: "none" }}>
              <div className="m-section__content">
                <h5 style={{ padding: 5, marginBottom: 5 }}>
                  ...
                  Backpack Weight : (LB){" "}
                  {Number(
                    parseFloat(this.state.backpackWeightInPound).toFixed(2)
                  )}{" "}
                </h5>
                <h5 style={{ padding: 5, marginBottom: 5 }}>
                  {" "}
                  Backpack Weight (KG):{" "}
                  {Number(
                    parseFloat(this.state.backpackWeightInKg).toFixed(2)
                  )}{" "}
                </h5>
                <h5 style={{ padding: 5, marginBottom: 5 }}>
                  {" "}
                ...
                <h5 style={{ padding: 5, marginBottom: 5 }}>
                  {" "}
                  Backpack Checkout Price : ($){" "}
                  {Number(
                    parseFloat(this.state.backpackCheckoutPriceUSD).toFixed(2)
                  )}{" "}
                </h5>
                <h5 style={{ padding: 10, marginBottom: 5 }}>
                  {" "}
                  SELLER Takes : (৳){" "}
                  {Number(
                    parseFloat(this.state.sellerTakesBDT).toFixed(2)
                  )}{" "}
                </h5>
                <h5 style={{ padding: 10, marginBottom: 5 }}>
                  {" "}
                ...
                <h5 style={{ padding: 10, marginBottom: 5 }}>
                  {" "}
                  Backpack Takes : (৳){" "}
                  {Number(parseFloat(this.state.backpackTakesBDT).toFixed(2))}
                </h5>
                <h5 style={{ padding: 10, marginBottom: 5 }}>
                  {" "}
                  Backpack Total : (৳){" "}
...
ReactDOM.render(
  <PriceCalculator />,
  document.getElementById("price_calculator_rc")
);
